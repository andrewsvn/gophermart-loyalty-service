...
func (us *UserPgStorage) GetUserByLogin(ctx context.Context, login string) (*model.User, error) {
	...
	rows, err := us.query(ctx, nil, sqlQuery, args...)
	if err != nil {
		return nil, fmt.Errorf("%w %s: %v", ErrExecuteSelect, userTableName, err)
	}
	defer rows.Close()

	return us.userFromRow(rows)
}

func (us *UserPgStorage) GetUserByID(ctx context.Context, id uuid.UUID) (*model.User, error) {
	sqlQuery, args, err := sqrl.
		Select(userColumns).
		From(userTableName).
...
func (us *UserPgStorage) CreateUser(ctx context.Context, login, authHash string) (*uuid.UUID, error) {
	newID := uuid.New()
	sqlQuery, args, err := sqrl.
	...
	_, err = us.exec(ctx, nil, sqlQuery, args...)
	if err != nil {
...
func (us *UserPgStorage) UpdateUserLoginTS(ctx context.Context, userID uuid.UUID) (bool, error) {
	sqlQuery, args, err := sqrl.
	...
	res, err := us.exec(ctx, nil, sqlQuery, args...)
	if err != nil {
...
func (us *UserPgStorage) CheckUserExistsByLogin(ctx context.Context, login string) (bool, error) {
	sqlQuery, args, err := sqrl.
		Select("ID").
		From(userTableName).
		Where(squirrel.Eq{"LOGIN": login}).
...
func (us *UserPgStorage) userFromRow(rows pgx.Rows) (*model.User, error) {
	if !rows.Next() {
		return nil, nil
	}
